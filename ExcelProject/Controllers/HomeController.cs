    ...
    public class HomeController : Controller
    {
        ...
        private Stream CreateExcelFile(Stream stream = null)
        {
            ...
            using (var excelPackage = new ExcelPackage(stream ?? new MemoryStream()))
            {
                ...
                // Thêm comment
                excelPackage.Workbook.Properties.Comments = "This is comment";
        ...
        {
            // Set defaults width cho tất cả column
            ...
            // Lấy range vào tạo format cho range dó từ A1 tới D1
            ...
            for (int i = 0; i < listItems.Count; i++)
            {
                ...
                // Format color nếu chưa thỏa điều kiện
                if (item.Money > 20050)
                {
                    // Ở đây sẽ format lại theo dạng fromRow, fromColm, toRow, toCol
            ...
            // Fix width của column với minimum width là 15
            ...
            //worksheet.Cells[listItems.Count + 4, 4].Formula = "SUM(D2:D" + (listItems.Count + 1) + ")";
            //// Hàm SumIf
            ...
            //// Column hiện tại lấy với địa chỉ (row hiện tại -1)/(row hiện tại -2) cùng 1 column
        ...
        private DataTable ReadFromExcelfile(string path, string sheetName)
        {
            ...
            // Load file excel và các setting ban dầu
            using (ExcelPackage package = new ExcelPackage(new FileInfo(path)))
            {
                ...
                // Khởi lấy sheet đầu tien trong file excel để truy vấn
                // Truyền vào name của sheet để lấy ra sheetcần
                // Nếu name = null thì lấy sheet đàu tiên
        ...
        {
            // Gọi lại để tạo file excel
            var stream = CreateExcelFile();
            // Tạo buffer memory steam để hứng file excel
            var buffer = stream as MemoryStream;
            // Content type dành cho file excel
            Response.ContentType = "application/vnd.openxmlformats=officedocument.spreadsheetml.sheet";
            // Hiện save as dialog chọn thư mục để lưu
            // File name của Excel này là excel demo
            Response.AddHeader("Content-Disposition", "attachment; filename=ExcelDemo.xlsx");
            // Lưu file excel như 1 mảng byte để trả về response
            Response.BinaryWrite(buffer.ToArray());
            // Send tất cả các output bytes về phía clients
            Response.Flush();
            Response.End();                                    
            // Redirect về index
        ...
        {
            var data = ReadFromExcelfile(@"D:\Development\Đồ án cơ sở\DanhSachSinhVien.xlsx", "First Sheet");
            return View(data);
        }
    }
}